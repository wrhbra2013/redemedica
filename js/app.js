...
function carregarEditorMenu(container) {
  ...
  menuDraft = navMeta.map(m => ({
    ...
    label: m.label,
    section: m.section,
    visible: m.id === 'menu' ? true : (m.visible !== false),
  }));
  ...
  container.innerHTML = `
    <div class="card">
      ...
      <div class="card-body">
        <p style="color:var(--text-secondary)">Use as setas para reordenar, renomeie ou oculte.
        As alterações valem para todos os visitantes assim que você salvar.</p>
        <div class="menu-editor-actions">
          <button type="button" class="btn btn-primary" onclick="salvarConfigMenu()">${ICONS.check}<span>Salvar menu</span></button>
